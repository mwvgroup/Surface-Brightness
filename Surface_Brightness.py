...
def flux_error(pho_val, pho_err, conv_val):
...
def surf_brightness_error(lum_val, lum_err, conv_val, conv_err):
...
def photometry(fits_file, radius, cord, red):
    ...
    Args:
        ...
        cord      (dict) : Dictionary of sn coordinates
                               {sn (str): coordinates (skycord)}
...
def create_tables(uv_type, directory, radius, cord, red, show_list = False):
    ...
    Args:
        ...
        cord      (dict) : A dictionary of sn coordinates
                               {sn (str): coordinates (skycord)}
    ...
    for fits_file in file_list:
        ...
        p = photometry(fits_file, radius, cord, red)
        for elt in p:
            if elt[0] == 'error':
                ...
                flux_err = flux_error(photom, photom_err, flux_conv)
                ...
                sbrightness_err = surf_brightness_error(lum, lum_err, arcmin, arcmin_err)
...
def create_plots(data_table, uv_type, radius):
